...
I didn't know we can assign an input an id and it's be included in event.targe.elements property. There we can select that input by id.

Another cool feature, which is React-specific, is useing ref to get a value of an input. Adding to an element ref attribute with a variable that’s essentially a variable defined by useRef(), it’ll update the current property of React.useRef() variable to the DOM’s element current value.

const inputRef = React.useRef(null);

<input ref={inputRef}>

Here, React will set up an inputRef.current to be the ref.current

So, overall, we can reference a user input like so:
inputRef.current.value

We can provide a default value for an input with defaultValue
## Background
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=amontrezor%40deloitte.ca).
